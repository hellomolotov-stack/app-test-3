...
export async function renderProfiles() {
    ...
    if (allCards.length === 0) {
        // Если нет ни одного профиля – показываем заглушки
    ...
    // Есть реальные профили – отображаем их все
    ...
    // Для гостей и владельцев без профиля – накладываем градиентный блюр на нижнюю половину
    // Создаём оверлей с мягким градиентом от прозрачного к размытому
    ...
    blurOverlay.style.pointerEvents = 'none'; // чтобы кнопки под ним были кликабельны
    blurOverlay.style.zIndex = '50';
    blurOverlay.style.background = 'linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.2) 30%, rgba(0,0,0,0.4) 100%)';
    blurOverlay.style.backdropFilter = 'blur(0px)';
    // Плавное нарастание блюра через градиент прозрачности (имитация)
    ...
    // Блокируем скролл, чтобы интерфейс был статичным
