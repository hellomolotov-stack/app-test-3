...
const REGISTRATION_API_URL = 'https://script.google.com/macros/s/AKfycbwrqPsMCMwHECRzAtL7PqX8k2UhZ9g0wC4S-YNnygFRmqJYwMgnK8Dnee9GB6kUqRmH6g/exec';
...
function showBottomSheet(index) {
    ...
    function updateContent() {
        ...
        // Изображение с контейнером для счётчика
        const imageHtml = hike.image ? `
            <div class="image-container">
                <img src="${hike.image}" class="bottom-sheet-image" onerror="this.style.display='none'">
                <div class="participant-counter" id="participantCounter">уже идут: ${currentParticipantsCount}</div>
            </div>
        ` : '';
        ...
        document.getElementById('prevHike')?.addEventListener('click', (e) => {
            ...
            if (sheetCurrentIndex > 0) {
                sheetCurrentIndex--;
                loadParticipantCount(); // обновляем счётчик для нового хайка
                updateContent();
    ...
    // Загружаем количество участников при открытии
...
// ----- Страница подарка (с меню для владельцев карты) -----
