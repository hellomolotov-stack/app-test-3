...
// Состояние бронирования для каждого хайка (ключ - индекс в hikesList)
...
async function loadHikes() {
    try {
        ...
        // Инициализируем состояние бронирования (по умолчанию false)
...
// ----- Массив партнёров -----
const partners = [
    // ... (полный массив, здесь не повторяем для краткости)
];
...
function showBottomSheet(index) {
    ...
    function updateContent() {
        ...
        document.getElementById('prevHike')?.addEventListener('click', (e) => {
            ...
            if (sheetCurrentIndex > 0) {
                sheetCurrentIndex--;
                updateContent();
                updateFloatingSheetButtons(); // обновляем состояние кнопок
                haptic();
    ...
    // Функция для обновления состояния кнопок (без пересоздания)
    function updateFloatingSheetButtons() {
        ...
        // Находим или создаём основные кнопки
        ...
        if (!goBtn) {
            // Если кнопок нет, создаём заново (но такого не должно быть, т.к. они создаются при открытии)
    ...
    function createFloatingButtons() {
        ...
        // Обработчик для кнопки "у меня вопрос"
        ...
        // Устанавливаем начальное состояние в соответствии с текущим хайком
        updateFloatingSheetButtons();
    }
